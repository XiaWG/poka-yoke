<template>
  <view style="flex-direction: column; flex: 1">
    <view class="body"> 
      ...
      <view class="title" @click="$refs.calendar.open()">
        日期: {{ date }}
      </view>    
...
export default {
  ...
  mounted () {
    this.date = this.$formatterTime('', true, 1)
    setTimeout(() => {
      this.focus()
    }, 100)
  },
    ...
    async handleGetScanData (data, name) {
      ...
      if (this.curInput === 1) { // 线体
        ...
        if (res && res.code === 0) {
          ...
          // })
          this.goToPage(`/pages/views/output/main?programId=${this.programId}&name=${this.productName}&productLine=${this.productLine}&date=${this.date}`)
          this.reset()
        }
      } else {
        // 校验登陆人        
