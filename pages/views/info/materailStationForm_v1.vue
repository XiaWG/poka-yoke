...
export default {
  ...
  computed: {
    placeholder () {
      ...
      return this.curInput === 1 ? '请扫描线体' : this.curInput === 2 ? '请登记或扫描产品名称' : '请扫描登录账号'
    }
  },
  watch: {
    loginUser (v) {
      if(v.length === 2) {
        this.goToPage(`/pages/views/info/infoCollectMain?curLine=${this.lineName}&name=${this.productName}&user=${JSON.stringify(v)}`)
        this.reset()
      }
    }
  },
