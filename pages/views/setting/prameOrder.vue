      ...
      <view v-if="logined">
        <view style="width: 100%;overflow: auto">
          <t-table min-width="400px">
            <t-tr>
              <t-th :width="80">物料参数</t-th>
              <t-th :width="80">聚合排序</t-th>
              ...
              >
                ...
                <t-td :width="80">
                  ...
                  <image
                    ...
                    @click.stop="handleUp(item, index, 'xxcj')"
                  />
                  <image
                    class="table-icon" 
                    :src="'../../../static/icon/down.png'"
                    @click.stop="handleDown(item, index, 'xxcj')"
                  />
                </t-td>
                <t-td :width="80">
                  <image
...
export default {
  ...
  methods: {
    ...
    handleUp (item, index) {
      ...
      item.orderIndex --
      this.mainList[index - 1].orderIndex ++
      this.mainList = this.mainList.sort((a, b) => (a.orderIndex - b.orderIndex))
    },
    handleDown (item, index) {
      if (index === (this.mainList.length - 1)) {
        uni.showToast({
          title: '已经是最后条了,无法下移',
          duration: 2000,
          icon: "none",
    ...
    handleDel (index) {
      for (var i = index; i < this.mainList.length; i ++) {
        this.mainList[i].orderIndex --
      }
    ...
    handleSubmitQX () {
      ...
      this.mainList.push({
        qx: this.qx,
        permision: [],
        orderIndex: (this.mainList.length + 1)
      })
